...
if ($id_venta != null) {
    ...
    $pdf->Cell(60, 5, utf8_decode('Fecha: ' . $result['fecha']), 0, 1, 'C');

    $pdf->Cell(60, 5, '===============================', 0, 1, 'C');
    //########## Datos del cliente
    $pdf->Cell(60, 5, utf8_decode('Atendido por: ' . $result['vendedor']), 0, 1, 'C');
    $pdf->Cell(60, 5, utf8_decode('Cliente: ' . $result['nombre']), 0, 1, 'C');
    ...
    $pdf->SetFont('Arial', '', 11);
    $total = 0;
    $moneda = empty($datos['moneda']) ? '$' : $datos['moneda'];
    foreach ($products as $product) {
        $total += $product['cantidad'] * $product['precio'];
        $pdf->Cell(20, 5, $product['cantidad'] . ' x ' . $moneda . $product['precio'], 0, 0, 'C');
        $pdf->MultiCell(40, 5, $product['descripcion'], 0, 'C');
        $pdf->Cell(60, 5, $moneda . number_format($product['cantidad'] * $product['precio'], 2), 0, 1, 'R');
        $pdf->Cell(60, 5, '------------------------------------------------------', 0, 1, 'C');
    }
    $pdf->Cell(60, 5, 'Total: ' . $moneda . number_format($total, 2), 0, 1, 'R');
    $pdf->Ln(2);
    $pdf->Cell(60, 5, 'Metodo', 0, 1, 'C');
    $pdf->Cell(60, 5, $result['metodo'], 0, 1, 'C');
    $pdf->Ln(2);
    $pdf->MultiCell(60, 5, utf8_decode($datos['mensaje']), 0, 'C');
